...
def call_gemini(prompt):
    ...
    try:
        ...
        log(f"Gemini API通信エラー: {e}")
        return None

def generate_posts(context_messages):
    """LLMを用いてX投稿のJSONを生成"""
    log("Gemini APIでX投稿ネタを生成中...")
...
def main():
    if not SLACK_TOKEN or not GEMINI_API_KEY:
        log("❌ 環境変数 SLACK_BOT_TOKEN または GEMINI_API_KEY が設定されていません。")
        sys.exit(1)
